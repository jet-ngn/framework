...
import AttributeList from '../data/AttributeList'
...
export function * getTemplateRenderingTasks ({ app, binding, view, element, template, childViews, routers, options, stagedViews } = {}) {
  const retainFormatting = element.tagName === 'PRE',
        { replace = false, append = false, mode = 'append', index = -1 } = options ?? {},
        { name } = view,
    ...
    yield [`Initialize "${name}" view content bindings`, async ({ next }) => {
      for (const id of keys) {
        await registerContentBinding({
          ...
          childViews,
          routers,
          options: { retainFormatting, mode, index }
        }).reconcile(true)
      }
