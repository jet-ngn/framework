...
import { escapeString } from '../utilities/strings'

export function reconcileNodes (original, update, binding) {
  const result = []
  ...
  for (let i = 0, length = Math.max(original.length, update.length); i < length; i++) {
    ...
    result.push(reconcileNode(existingNode, newNode, binding))
  }
...
function reconcileNode (original, update, binding) {
