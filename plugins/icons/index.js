export default function Icons (dependencies, cfg) {
  const name = 'Jet Icons'
  const { html, svg } = dependencies

  if (!html || !svg) {
    throw new Error(`${name} plugin requires the following dependencies: "html," "svg"`)
  }
    ...
    install (Plugins) {
      class IconManager {
        ...
        get (name) {
          ...
          `.setAttribute('class', [name, 'icon'])
        }
      }

      Plugins.icons = new IconManager(cfg)
    }
  }
}
